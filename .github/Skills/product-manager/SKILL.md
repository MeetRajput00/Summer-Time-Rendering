...
2. **Context Gathering**: Look through the application code. Use `list_dir`, `view_file`, or `grep_search` to understand the current state of the application. Understand what features currently exist and what is missing based on standard project requirements (e.g., in an RPG: auth, save states, physics, game loop). Find at least the requested number of actionable items.
